...
class Arithmetic(unittest.TestCase):
    ...
    def test_secfld(self):
        ...
        for a, b in zip(s + t, t + s):
            ...
            self.assertEqual(mpc.run(mpc.output(f * b)), a * b)
            self.assertEqual(mpc.run(mpc.output(a * g)), a * b)
            self.assertEqual(mpc.run(mpc.output(secpoly.if_else(True, f, g))), a)
            self.assertEqual(mpc.run(mpc.output(secpoly.if_else(False, f, g))), b)
            self.assertEqual(mpc.run(mpc.output(secpoly.if_swap(True, f, g)[1])), a)
            self.assertEqual(mpc.run(mpc.output(secpoly.if_swap(False, f, g)[1])), b)
            self.assertEqual(mpc.run(mpc.output(f // g)), a // b)
