...
class Arithmetic(unittest.TestCase):
    ...
    def _test_modall(self, poly):
        ...
        self.assertEqual(poly.from_terms(f'{X}'), p)
        self.assertEqual(int(poly(p)), p)
        self.assertEqual(int(poly(-p)), (p-1) * p)
        self.assertEqual(int(-poly(p)), (p-1) * p)
        self.assertEqual(poly(p).__repr__(), f'{X}')
        self.assertEqual(poly([0, 1, 0]), p)
        self.assertEqual(poly._to_list(poly(p)), [0, 1])
        self.assertEqual(poly.from_terms(f'{X} + {X}^2'), p + p**2)
        ...
        self.assertEqual(list(poly(1)), [1])
        self.assertEqual(poly(0).monic(), 0)
        self.assertEqual(poly(1).monic(lc_pinv=True), (1, 1))
        self.assertEqual(poly.from_terms(f'{X}+{X}^3').reverse(), f'{X}^2+1')
        ...
        self.assertEqual(poly(f'{X}^5+{X}^4+{X}^2+{X}+1').deriv(3), (60%p)*p**2 + (24%p)*p)
        self.assertEqual(poly(f'{X}^5+{X}^4+{X}^2+{X}+1').deriv(5), 120%p)
    ...
    def _test_errors(self, poly):
        ...
        self.assertRaises(IndexError, operator.getitem, poly(1), -1)
        self.assertRaises(IndexError, operator.getitem, poly(1), slice(0))
        self.assertRaises(IndexError, operator.getitem, poly(1), 0.5)
